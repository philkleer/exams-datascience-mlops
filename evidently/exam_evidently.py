...
def train_model(data: pd.DataFrame, numerical_features: list, categorical_features: list, target: str) -> pd.DataFrame:
...
def generate_report(reference_data: pd.DataFrame, current_data: pd.DataFrame, column_mapping, metrics):
...
def main():
    ...
    regressor, trained_data, tested_data = train_model(reference_jan11, num_feats, cat_feats, target)

    print('Creating model validation report ...', end='\r')
    # report1 = generate_report(trained_data, tested_data)
    report1 = generate_report(trained_data, tested_data, column_mapping, [RegressionPreset()])
    ...
    reference_jan11['prediction'] = regressor.predict(reference_jan11[num_feats + cat_feats])

    report2 = generate_report(reference_data=None, current_data=reference_jan11, column_mapping=column_mapping, metrics=[RegressionPreset()])
    ...
    for i, (start, end) in enumerate(weeks, 1): 
        ...
        report3 = generate_report(reference_jan11, week_data, column_mapping=column_mapping, metrics=[RegressionPreset()])
        ...
        # saving drift score
        mean_abs_error = report3.as_dict()['metrics'][0]['result']['current']['mean_abs_error']
    ...
    worst_week_df['prediction'] = regressor.predict(worst_week_df[num_feats + cat_feats])

    # Step 5
    # Target Drift Report on the worst week
    report_worst_target = generate_report(reference_jan11, worst_week_df, column_mapping, [TargetDriftPreset()])

    #report_worst_target.save_html('target_drift_report.html')
    PROJECT_NAME = "evidently_exam_task4"
    PROJECT_DESCRIPTION = "Evidently Exam DataScientest.com"
    add_report_to_workspace(workspace, PROJECT_NAME, PROJECT_DESCRIPTION, report_worst_target)
    ...
    last_week_data['prediction'] = regressor.predict(last_week_data[num_feats + cat_feats])

    # re-initaite Column mapping
    report_last_num = generate_report(reference_jan11, last_week_data, column_mapping=column_mapping, metrics=[DataDriftPreset()])

    PROJECT_NAME = "evidently_exam_task5"
    PROJECT_DESCRIPTION = "Evidently Exam DataScientest.com"
    add_report_to_workspace(workspace, PROJECT_NAME, PROJECT_DESCRIPTION, report_last_num)
