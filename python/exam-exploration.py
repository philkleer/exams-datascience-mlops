...
print('Answer to e): They are only approximately equal to the values of generating the sample, since the sample itself relies on a random sample strategy.')
...
for i, size in enumerate(nsizes):
    ...
    print('Iteration:', i, ', value of mean:', simulation.mean(), ', value of SD:', simulation.std())
    print('Iteration:', i, ', Diff to mu:', mu-simulation.mean(), ', value of SD:', sigma-simulation.std())
...
df_filter['categoryId'] = df_filter['categoryId'].replace(to_replace = [24, 10, 20, 27], value = ['Entertainment', 'Music', 'Gaming', 'Education'])
...
groupby_m = df_filter.groupby(
    [pd.Grouper(key = 'publishedAt', freq = 'm'), 
    df_filter['categoryId']]).agg({'likes':'mean'}).unstack().fillna(0)

groupby_m.plot(figsize = (20, 4.5), style = 'o-');

print('Overtime, we can see that videos dealing with music have the highest number of likes. The other three categories have only minor numbers of likes.\n')
print('Furthermore, we can see that the number of likes among the categories "Education", "Gaming", and "Entertainment" do not vary a lot over time. Instead, the number of likes for music videos varies greatly over teim from a low 100,000 (Dec 20) to a high 400,000 (Aug 20).')
print('For music videos, we can recognize that during the winter months (Dec, Jan, Feb) the average number of likes are lowest in 2021 and 2022.')
print('For educational videos, the number of likes are the lowest constantly over time among the four categories.')
...
print('From the result, we can see that there is a significant relationship between publication months and the category of video.')
...
categories = df_filter.groupby(
    [pd.Grouper(key = 'publishedAt', freq = 'm'), 
    df_filter['categoryId']]).agg({'video_id':'count'}).unstack().fillna(0)

categories.plot(figsize = (20, 4.5), style = 'o-')

print('The only thing that is visible is that videos in the category education constantly get similar counts, independently from the month.')
print('For the other categories, we can see that the count of videos from the categories entertainment, gaming and music varies over time.')
...
print('The distribution of "view_count" absolutely does not follow a normal distribution. ')
...
print('Conclusion, the p-value is lower than 0.05, therefore, we reject the Nullhypothesis. There is a significant difference between the videos enabled comments and disabled comments regarding the amount of likes ')

print('\n\n')
print('Mean for disabled comments:', df['likes'][(df['comments_disabled'] == True)].mean())
print('Mean for enabled comments:', df['likes'][(df['comments_disabled'] == False)].mean())
