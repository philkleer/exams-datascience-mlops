...
def valid_token():
    ...
    login_response = requests.post(login_url, headers={"Content-Type": "application/json"}, json=credentials)
    return login_response.json().get("token")

def test_valid_token(valid_token):
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {valid_token}"}
    response = requests.post(predict_url, headers=headers, json={"gre_score": 320, "toefl_score": 120, "university_rating": 2, "cgpa": 3, "research": 1})
...
def test_valid_prediction(valid_token):
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {valid_token}"}
    data = {"gre_score": 320, "toefl_score": 120, "university_rating": 2, "cgpa": 3, "research": 1}
...
def test_invalid_input_data(valid_token):
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {valid_token}"}
    invalid_data = {"gre_score": "invalid", "toefl_score": 120, "university_rating": 2, "cgpa": 3, "research": 1}
